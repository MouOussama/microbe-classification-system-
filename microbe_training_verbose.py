...
X_train_clean, X_test_clean, y_train_encoded, y_test_encoded = train_test_split(X_clean, y_encoded, test_size=0.1, stratify=y_encoded, random_state=42)
y_test_orig = label_encoder.inverse_transform(y_test_encoded)

os.makedirs('dataTest', exist_ok=True)
pd.DataFrame(X_test_clean).to_csv('dataTest/X_data.txt', sep='\t', header=None, index=False)
pd.DataFrame(y_test_orig).to_csv('dataTest/Y_data.txt', sep='\t', header=None, index=False)
print(f"   ✓ Test set saved to dataTest/ ({X_test_clean.shape[0]} samples)")

print(f"\n✅ Training on 90% ({X_train_clean.shape[0]} samples)")

# ===== PHASE 2: FEATURE ENGINEERING =====
print("\n" + "="*70)
print("PHASE 2: FEATURE ENGINEERING & PREPROCESSING (TRAIN 90%)")
print("="*70)
...
print(f"   ✓ Scaled train: {X_scaled.shape}")
print("   ✓ scaler.pkl saved")

print("🔄 Step 2.2: Statistical feature selection...")
selector = SelectKBest(f_classif, k=20)
X_selected = selector.fit_transform(X_scaled, y_train_encoded)
...
baseline_score = engineer.evaluate_feature_set(X_selected, y_train_encoded, "Baseline(top20)")
eng_score = engineer.evaluate_feature_set(X_engineered, y_train_encoded, "Engineered")
...
print("\n" + "="*70)
print("PHASE 3: ALGORITHM SELECTION (5-FOLD CV on train90%)")
print("="*70)
...
for name, model in models_config.items():
    ...
    fold_scores = []
    for fold, (train_idx, val_idx) in enumerate(cv.split(X_processed, y_train_encoded), 1):
...
print("\n" + "="*70)
print("PHASE 5: FINAL PRODUCTION TRAINING (TRAIN 90%)")
print("="*70)

print(f"🔄 Training final {best_name}...")
final_model = models_config[best_name]
final_model.fit(X_processed, y_train_encoded)
...
# Production pipeline (optional, for completeness)
...
print("="*70)
print(f"Train data: {X_train_clean.shape[0]} samples, {n_classes} classes (90%)")
print(f"Test holdout: {X_test_clean.shape[0]} samples (10%)")
print(f"Best Model: {best_name}")
print(f"Performance: F1-macro = {best_score:.4f}")
print(f"Test with Tab 6!")
print("\n🎉 TRAINING COMPLETE")
print("="*70)
